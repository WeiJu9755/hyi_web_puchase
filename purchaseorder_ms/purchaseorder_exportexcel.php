...
// 1. 先查主檔資料
// ==============================
$Qry="SELECT a.*,b.contract_abbreviation,c.short_name,d.supplier_name 
...
// 2. 查明細資料
// ==============================
$Qry="SELECT a.*,b.material_name,b.specification,b.unit,c.contract_id,
             d.unit_price AS contract_seq_unit_price,e.short_name
...
foreach ($pages as $pageIndex => $pageRows) {
    ...
    if ($pageIndex == 0) {
        ...
        $sheet = $objPHPExcel->createSheet($pageIndex);
    }

    buildPurchaseOrderSheet($sheet, $headerData, $pageRows, $pageIndex + 1, count($pages));
}
...
// function: 建立單頁採購單
// =====================================================
function buildPurchaseOrderSheet($sheet, $headerData, $pageRows, $pageNo = 1, $totalPages = 1)
{
    $purchase_order_id        = $headerData['purchase_order_id'];
    $contract_id              = $headerData['contract_id'];
    $requirement_description  = $headerData['requirement_description'];
    $created_at               = $headerData['created_at'];
    $contract_abbreviation    = $headerData['contract_abbreviation'];
    $short_name               = $headerData['short_name'];
    ...
    $sheet->getRowDimension(36)->setRowHeight(25);
    ...
    // 文件屬性
    // ==============================
    $sheet->getParent()->getProperties()->setCreator("PowerSales")
    ...
    $sheet->setCellValueExplicit('L10', $contract_id, PHPExcel_Cell_DataType::TYPE_STRING);
    ...
    for ($i = 0; $i < 15; $i++) {
        ...
        if (isset($pageRows[$i])) {
            ...
            $purchase_order_price += $total_price;
        } else {
            $material_no = "";
            $contract_seq  = "";
            $material_name = "";
        ...
        $sheet->setCellValue("A{$rowIndex}", $material_no);
    ...
    $sheet->getStyle('M31')->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_DOWN);
    $sheet->getStyle('M31')->getBorders()->getDiagonal()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
    ...
    $sheet->mergeCells('A33:B33'); $sheet->setCellValue('A33','申請人');
    $sheet->mergeCells('C33:E33'); $sheet->setCellValue('C33','工程師');
    $sheet->mergeCells('F33:H33'); $sheet->setCellValue('F33','部門主管');
    $sheet->mergeCells('I33:J33'); $sheet->setCellValue('I33','總務');
    $sheet->mergeCells('K33:L33'); $sheet->setCellValue('K33','總監');
    $sheet->mergeCells('M33:O33'); $sheet->setCellValue('M33','董事長');

    $sheet->mergeCells('A34:B34'); $sheet->setCellValue('A34','');
    $sheet->mergeCells('C34:E34'); $sheet->setCellValue('C34','');
    $sheet->mergeCells('F34:H34'); $sheet->setCellValue('F34','');
    $sheet->mergeCells('I34:J34'); $sheet->setCellValue('I34','');
    $sheet->mergeCells('K34:L34'); $sheet->setCellValue('K34','');
    $sheet->mergeCells('M34:O34'); $sheet->setCellValue('M34','');

    $sheet->mergeCells('A35:O35');
    $sheet->setCellValue('A35','採購流程核章(壓日期)');

    $sheet->mergeCells('A36:B36'); $sheet->setCellValue('A36','預計交期');
    $sheet->mergeCells('C36:E36'); $sheet->setCellValue('C36','採購');
    $sheet->mergeCells('F36:H36'); $sheet->setCellValue('F36','收貨');
    $sheet->mergeCells('I36:K36'); $sheet->setCellValue('I36','驗貨');
    $sheet->mergeCells('L36:M36'); $sheet->setCellValue('L36','交料 / 入庫');
    $sheet->mergeCells('N36:O36'); $sheet->setCellValue('N36','總務');

    $sheet->mergeCells('A37:B38'); $sheet->setCellValue('A37','');
    $sheet->mergeCells('C37:E38'); $sheet->setCellValue('C37','');
    $sheet->mergeCells('F37:H38'); $sheet->setCellValue('F37','');
    $sheet->mergeCells('I37:J37'); $sheet->setCellValue('I37','不合格/退換貨');
    $sheet->setCellValue('K37','合格');
    $sheet->mergeCells('L37:M38'); $sheet->setCellValue('L37','');
    $sheet->mergeCells('N37:O38'); $sheet->setCellValue('N37','');
    $sheet->setCellValue('K38','');
    $sheet->mergeCells('I38:J38'); $sheet->setCellValue('I38','');

    $sheet->mergeCells('A39:H39'); $sheet->setCellValue('A39','請款');
    $sheet->mergeCells('I39:O39'); $sheet->setCellValue('I39','備註');

    $sheet->mergeCells('A40:B40'); $sheet->setCellValue('A40','月份');
    $sheet->mergeCells('C40:E40'); $sheet->setCellValue('C40','狀態');
    $sheet->mergeCells('F40:H40'); $sheet->setCellValue('F40','資料上傳');
    $sheet->mergeCells('I40:O41'); $sheet->setCellValue('I40','');

    $sheet->mergeCells('A41:B41'); $sheet->setCellValue('A41','');
    $sheet->mergeCells('C41:E41'); $sheet->setCellValue('C41','');
    $sheet->mergeCells('F41:H41'); $sheet->setCellValue('F41','');
